...
rm_router = APIRouter(prefix="/room", tags=["Rooms"])
...
#CREATE ROOM
@rm_router.post("/create", response_model=RoomInternal, status_code=201)
async def create_room(service: T_Service, schema: RoomSchema) -> RoomModel:
  return await service.create_room(schema)


# GET ALL ROOMS
@rm_router.get("/list", response_model=RoomList)
...
# UPDATE ROOM
@rm_router.patch("/update/{room_id}")
...
# DELETE ROOM
@rm_router.delete("/delete/{room_id}")
