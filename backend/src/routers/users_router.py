...
async def create_user(service: T_Service, schema: UserSchema, admin: T_Admin) -> UserModel:
    return await service.create_user(schema)


# ========= READ ROUTER =========
@user_router.get("/", response_model=UserList)
async def list_users(service: T_Service, filter_page: T_FilterPage, admin: T_Admin) -> dict[str, Sequence[UserModel]]:
...
async def update_user(service: T_Service, schema: UserSchema, admin: T_Admin, user_id: int = Path()) -> UserModel:
    return await service.update_user(schema, user_id)


# ========= DELETE ROUTER =========
@user_router.delete("/{user_id}")
async def delete_user(service: T_Service, admin: T_Admin ,user_id: int = Path()) -> None:
